...
def fetch_archive_from_http(url: str, output_dir: str):
    if Path(output_dir).is_dir():
        logger.info(f"{output_dir} exists, skipping downloading data")
        return
...
p.add_component("splitter", DocumentSplitter(split_by="sentence", split_length=150, split_overlap=30))
...
results = p.run({"text_file_converter": {"sources": file_paths}})